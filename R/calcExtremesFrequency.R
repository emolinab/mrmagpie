...
#' @param percentage should the output be given as a percentage (TRUE) or years over xx years (FALSE) or as fraction ("fraction")
...
calcExtremesFrequency <- function(subtype = "MRI-ESM2-0:ssp370:multi", initialYear = 1995,
                                  yearsOver = 30, gcmAvg = FALSE, percentage = FALSE, source = "Biess2024") {
  

 if (source == "Biess2024"){  
  x <- readSource(source, subtype = subtype, subset = seq(initialYear - (yearsOver - 1), 2100, 1))
 } else {
